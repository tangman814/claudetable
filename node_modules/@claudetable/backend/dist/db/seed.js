...
`);
// Clear existing data
client_1.sqlite.exec("DELETE FROM reservation_tables");
client_1.sqlite.exec("DELETE FROM reservations");
client_1.sqlite.exec("DELETE FROM customers");
client_1.sqlite.exec("DELETE FROM tables");
client_1.sqlite.exec("DELETE FROM zones");
client_1.sqlite.exec("DELETE FROM settings");
// ─── Zones ───────────────────────────────────────────────────────────────────
const [zone1F] = client_1.db.insert(schema_1.zones).values({ name: "1F", description: "一樓室內", sortOrder: 1 }).returning().all();
const [zone2F] = client_1.db.insert(schema_1.zones).values({ name: "2F", description: "二樓室內", sortOrder: 2 }).returning().all();
const [zoneOut] = client_1.db.insert(schema_1.zones).values({ name: "外場", description: "戶外露台", sortOrder: 3 }).returning().all();
console.log("Zones created:", zone1F.id, zone2F.id, zoneOut.id);
// ─── Tables 1F ───────────────────────────────────────────────────────────────
const tables1F = client_1.db.insert(schema_1.tables).values([
...
// ─── Tables 2F ───────────────────────────────────────────────────────────────
const tables2F = client_1.db.insert(schema_1.tables).values([
...
// ─── Tables 外場 ─────────────────────────────────────────────────────────────
const tablesOut = client_1.db.insert(schema_1.tables).values([
...
// ─── Customers ───────────────────────────────────────────────────────────────
const [cust1] = client_1.db.insert(schema_1.customers).values({ name: "王大明", phone: "0912-345-678", notes: "VIP 常客", visitCount: 12 }).returning().all();
const [cust2] = client_1.db.insert(schema_1.customers).values({ name: "林小美", phone: "0923-456-789", email: "lin@example.com" }).returning().all();
const [cust3] = client_1.db.insert(schema_1.customers).values({ name: "陳建國", phone: "0934-567-890", notes: "海鮮過敏確認" }).returning().all();
const [cust4] = client_1.db.insert(schema_1.customers).values({ name: "張家豪", phone: "0945-678-901" }).returning().all();
// ─── Today's reservations (sample) ──────────────────────────────────────────
const _now = new Date();
const today = `${_now.getFullYear()}-${String(_now.getMonth() + 1).padStart(2, "0")}-${String(_now.getDate()).padStart(2, "0")}`;
const [res1] = client_1.db.insert(schema_1.reservations).values({
...
const [res2] = client_1.db.insert(schema_1.reservations).values({
...
const [res3] = client_1.db.insert(schema_1.reservations).values({
...
const [res4] = client_1.db.insert(schema_1.reservations).values({
...
client_1.db.insert(schema_1.reservationTables).values({ reservationId: res1.id, tableId: tables1F[3].id, capacityWarning: 0 }).run();
// res2 → A1 (2 pax, capacity 2, no warning)
client_1.db.insert(schema_1.reservationTables).values({ reservationId: res2.id, tableId: tables1F[0].id, capacityWarning: 0 }).run();
// res3 → C1 + C2 + C3 (22 pax, total capacity 8+8+... warning expected)
client_1.db.insert(schema_1.reservationTables).values({ reservationId: res3.id, tableId: tables1F[5].id, capacityWarning: 1 }).run();
client_1.db.insert(schema_1.reservationTables).values({ reservationId: res3.id, tableId: tables1F[6].id, capacityWarning: 1 }).run();
client_1.db.insert(schema_1.reservationTables).values({ reservationId: res3.id, tableId: tables1F[7].id, capacityWarning: 1 }).run();
// res4 → E1 (6 pax, capacity 6, no warning)
client_1.db.insert(schema_1.reservationTables).values({ reservationId: res4.id, tableId: tables2F[3].id, capacityWarning: 0 }).run();
// ─── Settings ────────────────────────────────────────────────────────────────
client_1.db.insert(schema_1.settings).values([
